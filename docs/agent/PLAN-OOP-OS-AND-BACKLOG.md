...
| ID | 内容 | 为什么必须在前 |
|---|---|---|
| **T0.1** | 把 3d 挪到 3f 之后的**切法变更**送 PR-Daemon 审，通过后改 SPEC §8 的 offer set 阶梯（`3c 空集 → 3e {Events,Approval} → 3d {+Memory}`） | 切法定错，后面四刀都长在错的形状上 |
...
| **T9** | **ME-3f 仓外包端到端 ← 这是验收** | T4,T5,T7,T8 | **黑盒**：先构建 daemon；之后生成并安装一个**仓库之外**的包，不改源码、不重新构建，重启后挂载 → 路由代理 → 事件转发全绿 |

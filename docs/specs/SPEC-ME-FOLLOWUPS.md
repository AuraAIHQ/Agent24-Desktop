...
### 第三轮挑战（2026-08-22）：又抓到一条 High，已修

| 复审结论 | 我的判定 | 处置 |
|---|---|---|
| `ON CONFLICT` 把**身份不一致也当成功**，只更新 `last_seen_at` | **成立，而且性质严重**：目录的全部价值就是归属权威，它却能在自相矛盾时返回 `Ok`，内核照样出借句柄，之后的数据继续记到旧身份上。"报成功的目录比没有目录更坏，因为调用方有权相信它" | upsert 加 `WHERE` 守卫 + 检查 `rows_affected`，不一致就是 `Conflict`。变异测试验证过 |
| 两个读方法**扫到短页才停**，并发写入下可能不终止；`limit == 0` 还要扫完整个分区 | **成立**。写入方持续 append 时每页都是满的，游标追着一条一直在动的尾巴 | **改成反向分页**：给 `EventQuery` 加 `newest()`(DESC) + `before(seq)`。游标只减不增，新写入落在它上方，**结构上就会终止**；`recent` 直接变成一条 `seq DESC LIMIT n`，代价与答案成正比而不是与分区成正比。`want == 0` 直接返回 |
| 0012 用了单参 `trim`（只去空格） | **成立**，正是 F5/#138 刚修过的那个坑，我在新表里又犯了一遍 | 改用 0011 的双参字符集 |
| 启动日志按 **module_name** 判断"是否在用" | **成立**。key 版本变更后同名模块会有新分区，按名字匹配会把历史遗留算成在用，恰好掩盖这个日志要暴露的东西 | 改按物理 key |
| 五个测试**删掉被保护的东西也照样过** | **成立**，逐条换掉：`no_scoped_memory_method_accepts_a_memory_id` 根本无法在运行期证明（已删除，改写进 trait 文档给后来加方法的人）；两个能力测试没有钉住 `granted`；幂等测试没有覆盖冲突元数据；`<= MAX_RESULTS` 在 520 行的分区上是空断言 | 新增：`limit == 0`、超过 `MAX_RESULTS`、并发写入下读必须终止（带超时）、冲突身份被拒、**记不下来就不出借**的端到端。四处变异验证均被抓到 |

### 第四轮复核（2026-08-22）：反向分页的连带影响

这一轮 codex CLI 在子代理沙箱里启动失败（`Operation not permitted`），实际是本地模型
用 git + 读文件做的复核，**强度低于对抗式外审**，开 PR 前需要补一次真的。结论仍有料：

| 结论 | 判定 | 处置 |
|---|---|---|
| 给 `EventQuery` 加字段后，`replay.rs::scan_page` **克隆整个 query**，`newest_first`/`before_seq` 会渗进 replay | **成立，是我引入的**。`.newest()` 会让第一页降序，`replayed_from_events` 取 seq 最大值，下一页问 `seq > max`，于是 **replay 一页就结束并倒序返回**，而且是静默的 | `scan_page` 里把这两个字段归零；文档写明 replay 自己拥有窗口与顺序。回归测试 + 变异验证 |
| `EventQuery::limit(-1)` 在 SQLite 里等于**无上限**，绕过 `DEFAULT_SCAN_LIMIT` | **成立**，而且直接打脸我刚加强的"每次 scan 都有界"这句话 | builder 和 bind 两处都 `.max(1)`（字段是 pub，能绕过 builder）。回归测试 + 变异验证 |
| 给 pub struct 加字段对下游是**源码级破坏** | **成立但无实际影响**：workspace 内无结构体字面量构造，该 crate 也未发布 | 不改 |
| `a_read_terminates_while_another_task_keeps_appending` **可能因为错误的原因通过** | **成立，而且比它说的更糟** | 见下 |
| 0012 注释"改名不会改变 key" | **写反了**。key 由名字派生，改名会产生**新** key，旧分区连数据留在原地 | 两处注释改正 |
| recall 注释"不会提前停止" | **与代码矛盾**，`hits.len() == want` 就返回 | 改成准确措辞 |

**关于那个并发终止测试，我自己的说法也要收回。** 按复审意见重写成"写入方不停 + 断言
读写重叠"之后，我把 `recall` 变异回正向分页——**测试照过，0.18 秒**。原因是：正向走法
要不终止，写入方必须在每读一页的时间里追加 ≥500 行，而实际每次往返只加一两行。所以

- 那条 Medium 在**形状**上是对的（循环没有上界），但"可以无限运行"够不着；
- 我的测试**证不了自己声称的性质**，已删除，改成一段说明为什么这里没有测试；
...
### 第六轮（终审，2026-08-22）：三条 Low，无 Medium/High

| 结论 | 判定 | 处置 |
|---|---|---|
| 模块级文档和测试注释**仍在说 id 带分区前缀、跨模块碰撞不可表示** | **成立**，是第五轮改完留下的过时注释。危险在于：后来的维护者据此可能去削弱 `append` 的冲突检查 | 改成"改进的是概率不是可能性"，并明写**真正兜底的是 append 拒绝同 id 不同 owner**，不要动它 |
| `recording_the_same_partition_twice` **没测 `last_seen_at` 会推进** | **成立**。把 upsert 改成 `DO NOTHING` 它照样过，而那会让每个反复挂载的分区的 `last_seen_at` 永远停在第一次——恰好是运维用来区分活分区和废弃分区的那一列 | 拆出独立测试，等 1.1 秒让秒级时间戳可区分。变异验证：改成不推进即 FAILED |
| `event.rs` 断言正向分页"**永远不会**终止" | **与我自己在 `os_memory.rs` 里的分析矛盾**——那里写的是要写入方快到能填满每一页才不终止 | 改成"可能不终止"，并说明这个界之所以值得，是因为它不依赖相对速度，而不是因为失败必然发生 |

Codex 同时确认：`mint_id` 去掉前缀后没有任何地方还依赖解析/匹配该前缀；migration 0012
与带守卫的 upsert 在**真实 WAL 磁盘库 + 多守护进程并发**下是稳的——身份校验与更新是
一条原子 SQLite 语句，`rows_affected == 0` 正确地只对应元数据不一致。

**六轮挑战到此收敛**（第 6 轮无 Medium/High），可以开 PR。

### 未在 F1 修的一条（→ 新增 F7）

复审指出：单条记忆的大小上限**不是配额**。模块可以用任意多次合法的小写入把共享
库撑大、抬高 WAL/checkpoint 成本、让每次 `recent`/`recall` 走更长的分区。这是真的，
代码注释里也是这么写的（"是地板，不是配额"）。真正的修法要按分区的字节/行数配额 +
限流 + 保留/压实策略，需要一张用量表，超出 F1 的范围。

**F7 · 领域 OS 记忆配额与保留策略**：分区级字节/行数上限、写入限流、过期/压实。
在有第二个真实领域 OS（Cos72）落地、或单机之外的多租户出现之前，优先级低于 F2。
...
见 F1 第三轮挑战一节：单条大小上限是地板不是配额，累积写入仍是跨模块的可用性通道。
需要分区级用量表 + 限流 + 保留策略。等 Cos72 落地后再排。
...
## FU-45 · 模块要不要知道请求的真实来源

ME-3b-4 把 `Forwarded` / `X-Forwarded-For` / `-Host` / `-Proto` / `X-Real-IP` **全部剥掉**了：客户端可以自带这些头，内核不重写，于是一个拿 `X-Forwarded-For` 做限流的模块信的是调用方自己填的值。剥掉也消掉了一处不对称——`Host` 被剥（模块看不到真实 authority），而伪造的 `X-Forwarded-Host` 原本会过去。

**剥而不铸是本轮的选择，不是终点。** 内核手上确实有对端地址（它拥有那条 TCP 连接），但目前没有任何模块要它，而**铸一个没人读的头是在发明契约**——一旦铸出来，它的语义、可信度和是否算「最小调用上下文」（§2）就都得当场定死，且没有一个真实消费者来检验定得对不对。

**到期条件是事件式的**：第一个真的需要知道调用者来源的领域 OS 出现时。届时要一起定：叫什么（大概率是 `X-A24-*`，因为那个前缀是唯一自动跟得上的剥离规则）、是地址还是一个不可关联的标识、以及它在多用户上线后与 §3 的 principal 租约是什么关系——**来源不等于身份**，把两者混成一个头是这类设计最常见的错。

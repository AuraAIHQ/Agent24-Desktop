...
模块要的东西对应 `Capability`。**本轮授予集是 `{Memory, Events, Approval}`**——`Models` / `Scheduler` / `Policy` 都不在本轮（§9）。
...
>
> 因此 `Capability` 枚举新增 `Approval`；`_a24/approval/request` 归它，未授予时稳定返回 forbidden。
...
初稿只有租约一种，于是把「模块写自己的私有分区」也一并要求了租约——**那是过严的**，并且会废掉领域 OS 最需要后台能力的那部分（定时清理、夜间巩固、异步计算）。分开：

| 授权 | 来源 | 能碰什么 | 谁能用 |
|---|---|---|---|
| **连接授权** | 模块连上回调 socket 时的令牌握手（模块身份 = 连接） | **只有自己的私有分区** `(启动时解析的 org, os:<module>)` | 请求内、后台任务都能用 |
| **请求租约** | 内核代理入站请求时注入的 `X-A24-Request-Id` | 用户空间 / shared space / 任何代表某个 principal 的操作 | **只有请求内** |

连接授权成立，是因为私有分区的三个分量今天没有一个需要请求上下文：`org` 是 daemon 启动时为 `LOCAL_USER` 解析一次并全程不变（`rust/apps/agent24d/src/domain.rs`），`space` 由模块名决定（`SpaceId::module_private`），模块名由连接决定。这与进程内模块拿到的句柄是同一个分区、同一套键——**进程外不该比进程内更严**。

代价要如实写：连接授权意味着**模块的后台任务可以在没有任何用户在场时写自己的库**。在 §0 的威胁模型下这不新增风险（它本来就能直接 open 那个 sqlite 文件），但 §5 的配额与限流**必须对后台路径同样生效**，否则一个刷屏的后台循环没人管。
...
| **ME-3c** | 回调通道：**NDJSON framing + 单行上限**、令牌握手、`initialize` 能力协商（授予集 = `{Memory, Events, Approval}`） | ME-3b | 超长行在解析前被拒并断连；未授予的能力对应方法稳定返回 forbidden；模块报的 manifest 摘要与内核不符时握手失败 | 实现者不需要猜 framing |
| **ME-3d** | 记忆回调 + **连接授权 / 请求租约两分**（§3）+ 真配额 + 双向上限 + page size/cursor + 可取消 | ME-3c | 无租约的**跨空间**回调被拒（自己的私有分区凭连接即可，且后台任务同样能写）；**请求结束后再用同一租约被拒**；过期租约被拒；**跨连接使用别的模块的租约被拒**；A 请求的租约落到 A 的 scope（拿不到别人的分区）；超配额写入返回明确错误；响应按 page size 分页（不是先构造再截断）；断连即取消 | 模块影响不了分区键 |
...
4. **崩溃退避与熔断参数**：需要一个真实的第三方模块才知道合理值。先与 F2 一致，不另立。

...
| `Content-Type` 的比较 | **每一个值**都比，且**不区分大小写** | `Text/Event-Stream` 和 `text/event-stream` 是同一个媒体类型；只看第一个值等于让模块用第二个值说了算 |
...
> | 刀 | 状态 |
> |---|---|
> | 3b-2a 版本协商 | **已合入 main**（PR #162，`agent24-os-proto::version`）—— 🟢 library-only，**不是 ✅** |
> | 3b-1 / 3b-2b / 3b-3 / 3b-4 / 3b-5 | 未开工 |
>
> 🟢 与 ✅ 的区别见 SPEC-MD-ME：🟢 = 库层可用但没有生产调用方，**不得记成已交付**。这张表要随每一刀落地更新；它没更新，本节的其余部分就开始骗人。

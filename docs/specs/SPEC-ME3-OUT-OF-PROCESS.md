...
| 刀 | 内容 | 依赖 | 独立可测的形式 |
|---|---|---|---|
| **3b-1** | NDJSON 单帧读取 + 单行上限，超限在**解析之前**拒并断连 | — | 只吃 `impl BufRead`，不认识 JSON 里有什么。喂超长首帧，断言 parse 从没被调用 |
| **3b-2** | 版本区间协商 + `initialize` 的请求/响应形状 + 能力协商响应形状（**本阶段 offer set 为空集**） | 3b-1 | 纯函数 `(模块区间, 内核区间) -> Result<选定版本, VersionMismatch>`，表驱动 |
| **3b-3** | spawn + 监督：起/停/崩溃退避/熔断/进程组终止/启动超时 | 3b-1,2 | **ready 这个概念到这一刀才存在** |
...
- **3b-2 的表驱动用例取自本文的版本矩阵原文，不自编。** 3b-1/3b-2 合入时没有生产调用方（SPEC-MD-ME 的 🟢），真风险不是「没有调用方」，是**唯一约束这个形状的东西是为这个形状写的测试**。用例与本文不符时改的是实现。

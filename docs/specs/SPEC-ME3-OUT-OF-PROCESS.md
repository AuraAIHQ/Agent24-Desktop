...
| 并发 permit 的寿命 | 跟着**响应 body**走，不跟着 handler 走 | 上限管的是**内存**，不是「同时有几个 handler 在跑」。handler 返回时那份缓冲还攥在手里，正被写给一个可能读得很慢、或者根本不读的客户端。在 `return` 处释放，一千个慢读者各攥一兆而上限一个都没数到 |
| 超时文案 | 三个时限**各自的文案**：客户端 body 没收完 / 模块没开始答 / 模块没答完 | 共用一条文案会**报错限**（head 触发却写 30s），对慢客户端还会**报错责任方**（写成「模块没答」）。§2.1 说这两个时限回答不同的问题——那就得答得出是哪一个 |

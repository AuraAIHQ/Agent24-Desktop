...
- **3b-1/3b-2 的错误必须是 typed，不能是 `String`。** 「握手期任何失败都断连」这条规则的家在 3b-3（连接的所有权在那里），而 3b-1 是 `BufRead`、3b-2 是纯函数，两者都不拥有连接。若它们只交出字符串，3b-3 只能一视同仁地断连，操作者看到的消息分不开「帧超长」和「版本不兼容」—— 那正是 #159 门 6 那条故事的形状：**一个真实的区别被压进一个共用的表达。** 这一条是判据 2 施加在这套切法上时掉出来的第一个结果。
- **3b-2a 的表驱动用例取自本文的版本矩阵原文，不自编。** 3b-1/3b-2 合入时没有生产调用方（SPEC-MD-ME 的 🟢），真风险不是「没有调用方」，是**唯一约束这个形状的东西是为这个形状写的测试**。用例与本文不符时改的是实现。
- **3b-1/3b-2 的 PR 里要写出 3b-3 的调用点桩**（构造 reader + negotiator，其余 `todo!()`），**让签名在消费者存在之前先跟消费者碰一次**；并带一个到期日（形状照 `install.rs` 那条 `TEMPORARY`：3b-3 若 N 天内未落地，3b-1/3b-2 退出 main）。
...
- **凡是要上 wire 的常量串/码，必须有一条测试把 SPEC 原文作为引文写在断言旁，断言该常量以*整词形式*出现在引文里。**
  > **判据**：把该常量改成引文里的一个**短子串**（`version_mismatch` → `version`）必须使测试变红；**对照** = 不改时绿。

  第二句是承重的那句：没有它，「有一条测试」和「有一条**会响的**测试」分不开，而照抄的人写个 substring 断言也算满足。3b-2a 上实测过这个区别 —— 第一版只排除了一个特定拼写错误，`KIND = "version"` 照样全绿，而 `version` 正是这个串最可能的截断形式。SPEC 把这些串写在反引号里，所以反引号就是词边界。同形状适用于 `-32700` / `-32600` / `-32000` / `auth_failed` / `manifest_mismatch`（即使实现成 `i32`，`contains(&format!("`{n}`"))` 仍可用）。

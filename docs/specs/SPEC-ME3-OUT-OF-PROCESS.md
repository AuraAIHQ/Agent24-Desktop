...
| **ME-3b** | 受约束代理（§2）+ 进程监督（起/停/崩溃退避/熔断/进程组终止）+ **完整的握手**,含它自己需要的那部分 wire:NDJSON **单帧**读取与单行上限、`initialize` 的请求/响应形状、握手期错误（`-32600` / `auth_failed` / `manifest_mismatch` / `version_mismatch`）及其测试、版本区间协商、能力协商的响应形状 —— **不含任何业务方法、不含多请求并发/取消**。

（**两轮修正的结果**：初版把 `initialize` 整个放在 ME-3c 而 ME-3c 依赖 ME-3b，ME-3b 手上没有本文定义的唯一 ready 判据；上一版只把「握手」挪过来，但读一条 `initialize` 同样需要 framing、上限和错误闭集 —— 那些还留在 3c，于是 3b 只能临时猜一种 framing 再由 3c 重写。**凡 ME-3b 判 ready 所必需的 wire，一律并入 ME-3b。**） | ME-3a | mock 后端**回显它实际收到的 header**，断言 bearer 与客户端伪造的 `X-A24-*` 都看不见；无 token 访问模块路由 401；越命名空间的 `Location` 不被跟随；**恶意模块在响应里回显 `X-A24-Approval-Token` / `X-A24-Request-Lease` / 任意 `X-A24-*`,客户端侧收不到**；子进程被 kill 后自动重启、期间该命名空间 503（不是 500/挂起）；快速崩溃触发熔断；**握手侧（判 ready 所必需，因此归本项而非 ME-3c）**：超长**首帧**在解析前被拒并断连；`initialize` 非首条 / 重复 → `-32600` 断连；认证失败 → `-32000` + `auth_failed` 断连；模块报的 manifest 摘要与内核不符 → `-32000` + `manifest_mismatch` 断连；**版本协商矩阵**：双方各报区间 `[min,max]` 取交集，交集为空 → 握手失败并把两边区间都放进错误（`-32000` + `version_mismatch`），交集非空 → 选定唯一版本 `min(模块.max, 内核.max)` 并在响应里回显，**内核不得回一个模块没声明支持的版本**；模块**不报区间** → 视为不兼容，握手失败；能力协商的响应形状可断言，且**能表达「本 daemon 不提供 `memory.scoped` 这个方法族」** | 模块看不到内核凭据；一个模块挂掉不带走内核（broker 层）；**ME-3b 能独立、安全地判 ready** |
| **ME-3c** | 回调通道的**其余部分**：多请求**并发与乱序**、重复 id、`$/cancelRequest`、超时、连接关闭语义、**业务层错误闭集**（握手所需的 framing / 上限 / 握手错误 / 版本与能力协商已在 ME-3b） | ME-3b | **握手后**的超长行被拒并断连；并发在途请求按 id 正确配对、响应可乱序；仍在途的 id 被复用 → 该请求失败；`$/cancelRequest` 使目标请求回 cancelled 响应；**连接断开则在途请求就地中止、不产生响应**；超时不重试；**params 解析失败固定返回 `-32602` 且不 dispatch handler**；**一条坏 params 只失败该行 RPC，连接继续处理下一行**（只有 framing 超限才断连）；**重复的 JSON object key 被拒**（否则先解析成 Map 会丢掉「这个字段出现过几次」这一事实）；错误形状按 §3 裁决表的闭集 `error.data.kind` 断言（**不是由实现 PR 现定**）；**此阶段 offer set 为空，所以调用任何业务方法都应是 `-32601`（「本 daemon 不提供」），不是 forbidden** —— 不得为了凑一条 forbidden 测试而提前注册方法（那会与「未实现的 `scoped/*` 必须 method-not-found」直接打架）。「有 handler 但调用方没拿到 grant → forbidden」归 ME-3d/3e | 实现者不需要猜并发、取消与错误分类 |

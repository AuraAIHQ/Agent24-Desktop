...
**不铸合成 `run_id`**，理由不是洁癖：`ApprovalRequest.run_id` / `Approval.run_id` 都是必填 `String`，而**渠道桥把它当路由键**——微信桥用 `a.run_id === result.runId` 决定这条审批推给哪个用户（`packages/wechat-bridge/src/bridge.ts`），Nostr 桥按 npub 分 session 同理。往那张表里塞一个合成 id，桥会**试图把审批推给一个从没提过任何要求的用户**。那是 bug，不是不优雅。

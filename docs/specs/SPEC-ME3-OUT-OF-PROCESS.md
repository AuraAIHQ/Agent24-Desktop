...
### 2.1 实现时必须定死的几条（ME-3b-4 落地时回写）

上面那一节说清了「剥什么、注什么」，但没说清下面这些。它们不是细节：**每一条如果留给实现者猜，第二个实现会猜出另一个答案，而两边都能声称「符合 §2」。**

| 项 | 裁决 | 为什么不是另一个答案 |
|---|---|---|
| 上游地址的类型 | **`SocketAddr`，不是 URL** | §1 说「模块不选地址」。用 `SocketAddr` 之后，「模块指定一个主机名」不是被挡住了，是**不可表达**——这条比任何校验都强 |
| `x-forwarded-` **整个前缀** + `Forwarded` / `X-Real-IP` | **入站一律剥掉**，且**本轮不由内核铸** | 客户端可以自带，内核不重写，于是一个拿 `X-Forwarded-For` 做限流、或用 `X-Forwarded-Host` 拼链接的模块，信的是**调用方自己填的值**。这也是不剥它就会留下的不对称：`Host` 被剥掉（模块看不到真实 authority），伪造的 `X-Forwarded-Host` 却过去了。**剥而不铸**：内核手上确实有对端地址，但还没有任何模块要它——铸一个没人读的头是在发明契约（FU-45） |
| 为什么这一族用**前缀**而内核私有头用**名单** | 因为**内核是唯一一跳** | 本文别处说的是「前缀是强形式、名单不会自动跟上」，这里必须说清为什么同一个理由在这一族上得出的是前缀。初版写死了「显而易见的三个」，而它漏掉 `X-Forwarded-Ssl` / `-Scheme`（Rack 的 `Request#ssl?` 直接读这两个，模块会以为连接是 TLS）与 `-Port`（Symfony 的 trusted-proxy 路径用它拼链接）。前缀在这里之所以安全，是因为**到达模块的请求上不可能存在一个正当的 `x-forwarded-*`**——内核是唯一一跳，所以前缀没有可误伤的对象。代价与 `X-A24-*` 那条同形：模块不得给自己的头起名叫 `x-forwarded-*`，而这是一条人能遵守的规则。**反过来，`Authorization` / `Cookie` 那一族没有这样的共同前缀，所以那里只能是名单——弱处仍在，见本节末** |
| 转发给模块的 path | **完整路径，含 `/api/v1/<ns>` 前缀**，不剥 | 剥掉前缀后模块**无法在自己的响应体里生成正确的链接**（它不知道自己被挂在哪儿），而响应体内核不重写。代价如实写：**进程内模块的 handler 看到的是剥掉前缀的路径**（`Router` 被 `nest`），所以同一个模块在两种形态之间迁移时路由要改。这是一处真实的不对称，不是疏忽 |
| 越命名空间的 `Location` | **整个响应变 502**，不是「把那个头删掉」 | 一个 `302` 而 `Location` 不见了，客户端拿它什么也做不了；**悄悄变成另一个东西比报错更糟** |
| 带 scheme 的绝对 URL | **一律拒，包括指向本 daemon 自己的** | 判断 `http://127.0.0.1:8080/...` 是不是「我们自己」，要知道 daemon 对外的 authority——在反向代理后面它并不知道 |
| `%2e` 与反斜杠 | `%2e` **先解码成 `.` 再规范化**；`\` 出现在任何位置**一律拒** | 浏览器两样都会折叠。`/api/v1/<ns>/%2e%2e/%2e%2e/admin` 只在浏览器那一侧真的越界，而那正是唯一要紧的一侧。代价：路径段里真想放一个字面 `%2e` 的模块会被拒——这个取舍值得写出来，而它就是这一个 |
| 控制字符与空白 | `Location` 里出现**任何**控制字符或空白**一律拒** | 浏览器解析 URL 时会**删掉** ASCII tab 和换行。`/api/v1/<ns>/<TAB>..<TAB>/runs` 在这条规则里是命名空间内，在浏览器那里是 `/api/v1/runs`；`h<TAB>ttp://evil.example` 在这里是相对路径，在那里是绝对 URL。**被判的那个串不是被执行的那个串**——本文一路在抓的就是这一个形状 |
| 重复的 `Location` | **超过一个值 → 502**，即使两个都在命名空间内 | `HeaderMap::get` 取第一个，而剥头是 `get_all` 全部转发：**判的值不是走掉的值**，一个无害的第一条后面可以跟一条越界的。而客户端对「哪个重复值算数」并不一致，「你指哪一个」没有安全的默认答案 |
| `Location` 的检查时机 | **任何状态码都查**，不只 3xx | `201 Created` 上的 `Location` 一样会被交给客户端 |
| `Refresh` 响应头 | **一律剥掉**，不走 `location_within` 的检查 | `Refresh: 0; url=http://evil.example` 不是标准头，但四大浏览器都认，语义等于 `<meta http-equiv="refresh">`。它**导航**，于是干着 `Location` 的活，而上面那一整套（scheme、`//host`、`%2e`、tab、重复值）一条都不经过。**一条点名了某个头的规则，不是一条关于「重定向」的规则** —— §2 原来那句「内核自己的 surface 绝不携带这个跳转」在它面前是假的。选择剥掉而不是纳入检查：纳入检查等于承认它是一种受支持的重定向机制，而 `Location` 已覆盖全部正当用途。（`Content-Location` 不引发导航，不管。） |
| `Content-Type` 的比较 | **每一个值**都比；比的是 `;` 之前的 **media type 整串**（不区分大小写），**不是前缀** | `Text/Event-Stream` 和 `text/event-stream` 是同一个媒体类型；只看第一个值等于让模块用第二个值说了算。而用前缀比会把 `text/event-streaming` 也拒掉——**错拒一个合法响应和错放一条流一样是缺陷**，第二实现在「前缀 / 全串 / MIME 解析」之间没有理由自己挑 |
| `Transfer-Encoding: chunked` 的响应 | **有限的照常放行**（缓冲后重建 framing 再发出），**不在 head 拒** | 这一条**修改了 §9 那句「SSE / chunked / WebSocket 本轮拒绝」的读法**，所以理由写全：本轮不做的是**流式转发**，而这里本来就是缓冲；`text/event-stream` **声明了**「我是一条流」，所以能在 head 就明确告诉模块不行，而 `chunked` 什么也没声明——它只是服务端没有预先算出长度时的常规发法。按字面拒会换来措辞上的合规，代价是拒掉能正常工作的模块。**留下的那一格如实写**：没有 SSE content-type 的**无尽** chunked 响应在 head 判不出来，由 1 MiB 上限（502 `upstream_response_too_large`）或总时限（504）兜住，不是当场 502 |
| 并发 permit 的寿命 | 跟着**那块字节**走（permit 放进 `Bytes` 的 owner 里），既不跟着 handler 也不跟着响应 body | 上限管的是**内存**，不是「同时有几个 handler 在跑」。这一条错了两次，方向相同——**permit 在它本该计数的那块内存还活着的时候就还回去了**：① 在 handler `return` 处释放，而那份缓冲正被写给一个可能读得很慢、或根本不读的客户端；② 在响应 **body** 被 drop 时释放，而 body 是把 `Bytes` 交到一个 frame 里的，hyper 拿走 frame 之后就把 body 丢了,那一兆还在它的写队列里。绑到字节的 owner 上才终止这个递归:**最后一个 clone 消失的那一刻就是内存真的没了的那一刻**，而且不需要任何代码记得去点名它 |
| 空路径的 `Location` | `?query` / `#frag` / 空串 → **保留本次请求的 path**（RFC 3986 §5.3），一律算命名空间内 | 按「基准目录」解析会把最后一段丢掉:`/api/v1/<ns>` + `?page=2` 变成 `/api/v1/`,于是一条再普通不过的翻页链接成了 502。**错拒一个合法响应也是缺陷**,不是安全上的保守 |
| 超时文案 | 三个时限**各自的文案**：客户端 body 没收完 / 模块没开始答 / 模块没答完；head 那条报的是**实际生效**的那个预算（head 与「总时限还剩多少」取小） | 共用一条文案会**报错限**（head 触发却写 30s），对慢客户端还会**报错责任方**（写成「模块没答」）。而写死 head 常量同样会报错：总时限比 head 短时，1 秒就放弃却写着「10 秒内没答」——**一条指错旋钮的消息把读者送到错的地方** |
| 响应头的判定时机 | 状态码 / 流式 / `Location` 一律在**读 body 之前**判 | 一条无尽的 SSE 正是这条规则要拒的东西。先缓冲再判，答案会是 `504 upstream_timeout`——「模块很慢」的码，用在一个**瞬间就答了**的模块身上。**检查必须在它要防的那件事的上游** |
| `X-A24-Request-Id` | **计数器 + 每次 daemon 启动一个随机前缀，不是秘密** | §2 已把它和 `X-A24-Approval-Token` 分开了；相关性 id 便宜是那次拆分的收益。**要不可猜的是后者**，它在 ME-3e 铸，永远不在代理里铸 |
| 状态码分工 | 上游不可达 / 响应中途断 / 响应超上限 / 越界或重复 `Location` / 流式 → **502**；时限到 → **504**；**503 = 这个命名空间此刻收不下这个请求**——DRAINING、还没 ready（§4）、以及过载，三者由 `error.code` 区分 | §8 要求 disable 与重启期间该命名空间答 **503**。502 与 503 混用，操作者就分不开「模块崩了/在重启/满了」与「模块答了但答得不合法」。**503 不是只留给 §4**——「满了」和「正在停」对客户端是同一件事（等一下再来），对操作者不是，所以分在 `code` 上而不是分在状态码上 |
| 时限 | **两个**：总时限（**从读客户端 body 起算**，30s）与首字节时限（响应 head，10s） | §5 要的就是这两条，而它们回答不同的问题：接了连接却不出 head 的模块是卡死了，还在发 body 的模块是在干活；只有一条时，两者是 30 秒之后同一个读数。总时限**必须**把读客户端 body 算进去——一个每次一字节喂 chunked body 的客户端，和一个卡死的模块一样能把 daemon 钉住，而从上游调用才开始计时的时限**看不见它** |
| 并发上限 | 每模块 **64** 个在途请求，超出**当场拒**（503 `module_overloaded`），**不排队** | 每个在途请求都攥着自己缓冲的 body，没有上限时 daemon 内存的界是**调用方的耐心**。§8 说「一个模块挂掉不带走内核」，这个常量是让那句话为真而不是一种指望的主要部分。排队不算数：队列就是调用方控制的内存，正是要被限的那个东西 |
| 上限与时限的文案 | 一律报**当时实际生效**的那个值，不报常量 | 和上面「超时文案」同一条，只是长在另一个地方：过载文案原本写死 `MAX_INFLIGHT_PER_MODULE`，而上限其实来自每模块的状态。生产两者相等，所以它**还不是错的，只是没有任何东西守着**——做成每模块可配的那天，文案开始撒谎而不会有人知道 |
| body 上限 | 请求与响应**都**用内核自己的 `MAX_BODY_BYTES` | §5 的「双向上限」在代理侧的那一半。用模块选的上限等于让不可信的一方决定花掉 daemon 多少内存 |

**一条要如实说的弱处**：`Authorization` / `Cookie` 这类内核私有头靠**一张写死的名单**挡，`X-A24-*` 靠**前缀**挡。前者不会自动跟上——明年加的一个内核私有头，如果不叫 `X-A24-*`，也不在名单里，它会直达模块。**所以内核私有头必须叫 `X-A24-*`**：让它成立的是这条命名约定，不是代理的代码。这句话的家在 `server.rs` 那个 auth 中间件旁边，因为加头的人在读那里。

**做不到的**：模块永远可以在**响应体**里跳转（meta refresh、一行脚本）。`Location` 那条规则管的是内核会替它递出去的那个头，不是「让跳转不可能」——模块本来就拥有自己命名空间里的内容。
...
> | 刀 | 状态 |
> |---|---|
> | 3b-1 framing / 3b-2a 版本协商 / 3b-2b `initialize` 线格式 | **已合入 main**（#161 / #162 / #164）—— 🟢 library-only |
> | 3b-3 spawn + 进程监督 | **已合入 main**（#170 / #171，`launch` + `supervise`）—— 🟢 |
> | 3b-4 受约束代理 | **本 PR**（`agent24-os-proto::proxy`）—— 🟢，生产挂载点还在 3b-3 的监督器手里 |
> | 3b-5 两阶段热 disable | 未开工 |
>
> 🟢 与 ✅ 的区别见 SPEC-MD-ME：🟢 = 库层可用但没有生产调用方，**不得记成已交付**。
>
> **这张表本身就是它警告的那个东西。** 上一版写「3b-1 / 3b-2b / 3b-3 / 3b-4 / 3b-5 未开工」，写的时候是对的，而在 #164 合入的那一刻起有三行是错的——没有任何东西让它会过期。**它的替代品不是一张更新得更勤的表，是一条能跑的命令**：`bash docs/agent/me3-status.sh`（#165）读 `origin/main` 上某个符号在不在，自带正负对照。那个 PR 合入后，这一段整个由探针取代——**若与本表冲突，取探针**。
...
- **不做流式代理。** WebSocket 与 SSE 本轮拒绝，不含糊放行（`101` 与 `text/event-stream` 在响应 head 就拒，502）。**`chunked` 是例外，读法见 §2.1**：本轮不做的是流式**转发**，而代理本来就缓冲；`chunked` 不像 `text/event-stream` 那样声明自己是流，按字面拒会拒掉正常模块。有限的 chunked 响应照常放行，无尽的由 body 上限与总时限兜住。（ME-3b-4 落地时改的这一句读法——**代码与本文在这一条上必须一致**，不然照本文写第二实现的人会造出一个拒绝正常模块的代理。）

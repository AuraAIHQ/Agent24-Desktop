...
| `X-Forwarded-*` / `Forwarded` / `X-Real-IP` | **入站一律剥掉**，且**本轮不由内核铸** | 客户端可以自带，内核不重写，于是一个拿 `X-Forwarded-For` 做限流、或用 `X-Forwarded-Host` 拼链接的模块，信的是**调用方自己填的值**。这也是不剥它就会留下的不对称：`Host` 被剥掉（模块看不到真实 authority），伪造的 `X-Forwarded-Host` 却过去了。**剥而不铸**：内核手上确实有对端地址，但还没有任何模块要它——铸一个没人读的头是在发明契约（FU-45） |
